...
## ⚠️ 구현 전 필수 게이트 — Simplicity First

**코드 작성 전에 아래 항목을 먼저 확인한다. 하나라도 위반이면 team-lead 승인 후 착수.**

- [ ] 이 변경이 현재 요청 범위 내인가? (요청하지 않은 기능/추상화 추가 금지)
- [ ] 새 함수/훅/Context/클래스/레이어를 만든다면, **사용처가 3개 이상인가?**
  - grep으로 사용처 먼저 확인. 3개 미만이면 인라인 유지 or team-lead 확인
- [ ] "나중에 필요할 것 같아서"가 이유인 설계가 아닌가?
- [ ] 표준 ORM/프레임워크 관례를 SOLID 이유로 변경하는 경우, 명확한 이득이 있는가?
  - 예: Flask 모델의 set_password/check_password — 표준 관례, 제거 대상 아님
  - 예: Flask Blueprint 구조 — AOP 없이 Service Layer 강제는 오버엔지니어링
