...
| 엔드포인트 | 권한 | 설명 |
|-----------|------|------|
| `GET /api/posts` | 공개 | published 포스트 목록. 파라미터: `?page=1&per_page=20&q=검색어&visibility=public&category_id=5&author=username` (페이지네이션+검색+필터) |
...
| `PUT /api/auth/me` | 로그인 | 프로필 수정 (username, email, password, bio, avatar_url) |
| `GET /api/users/:username` | 공개 | 유저 블로그 프로필 조회 (id, username, bio, avatar_url, post_count, created_at). 404: 없음/비활성화된 사용자 |

### 카테고리 API

| 엔드포인트 | 권한 | 설명 |
|-----------|------|------|
| `GET /api/categories` | 공개 | 카테고리 flat list (페이지네이션). 파라미터: `?page=1&per_page=50`. 응답: id, name, slug, description, parent_id, post_count, created_at |
