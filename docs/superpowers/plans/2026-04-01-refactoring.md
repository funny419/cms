...
PostEditor.jsx에서 폼 상태 + localStorage 자동저장 + API 호출 로직만 훅으로 추출.
PostEditor.jsx는 UI 렌더링만 담당. 419줄 → 약 200줄 목표.

**⑤ BlogLayout 카테고리 필터 Context 이동 (FE-P3)**

`categoryId`, `setCategoryId`를 별도 Context로 이동.
Layout 컴포넌트의 props를 `posts`, `loading`, `hasMore`, `sentinelRef` 4개로 축소.
...
| 단계 | 작업 | 파일 | 영향도 | 리스크 |
|------|------|------|--------|--------|
| **FE-P1** | `useAuth.js` 신규 | hooks/useAuth.js | 15개 파일 token/user 접근 정리 | 낮음 |
| **FE-P1** | `api/client.js` 신규 | api/client.js | 9개 api/*.js authHeader 통합 | 낮음 |
| **FE-P2** | `useFetch.js` 신규 | hooks/useFetch.js | 5개 컴포넌트 async 패턴 통합 | 낮음 |
| **FE-P3** | `usePostEditor.js` 추출 | hooks/usePostEditor.js | PostEditor.jsx 분해 | 중간 |
| **FE-P3** | BlogLayout props 축소 | BlogHome.jsx + layouts/* | Props Drilling 해소 | 중간 |
...
### 4-1. SRP — 공통 헬퍼 함수 추출 (신규 파일: `backend/api/helpers.py`)
...
새 필터 추가 시 기존 함수 수정 없이 새 함수만 추가.

### 4-3. DIP — Service Layer 도입 (선택적, 장기)

현재 단계에서 전면 도입은 오버엔지니어링. `stats.py::_get_stats()`, `series.py::_check_owner()` 패턴처럼
**핸들러 내 헬퍼 함수 수준의 분리**부터 적용 권장. Repository Pattern은 테스트 커버리지 확보 후 검토.

**LSP/ISP:** `StorageBackend` 추상 클래스가 이미 적절히 구현됨. Flask Blueprint 구조상 추가 적용 범위 제한적.
